...
export class MemoryGame extends BaseGame {
  ...
  private startRound(io: Server, room: Room) {
    ...
    room.gameState.playerProgress = room.players.reduce((acc, p) => ({ ...acc, [p.id]: 0 }), {});
  ...
  handleInput(io: Server, socket: Socket, room: Room, data: any) {
    ...
    if (room.gameState.eliminatedIds.includes(socket.id)) return;

    const { step } = data;
    const progress = room.gameState.playerProgress[socket.id];
    const expected = room.gameState.sequence[progress];

    if (step === expected) {
      room.gameState.playerProgress[socket.id]++;
      
      if (room.gameState.playerProgress[socket.id] === room.gameState.sequence.length) {
        // Player completed sequence
        const player = room.players.find(p => p.id === socket.id);
        if (player) player.score += 10 * room.gameState.round;
        ...
        const finishedCount = Object.keys(room.gameState.playerProgress).filter((pid) => {
           return room.gameState.playerProgress[pid] === room.gameState.sequence.length || room.gameState.eliminatedIds.includes(pid);
        }).length;

        if (finishedCount === survivors.length + room.gameState.eliminatedIds.length) {
          this.nextRound(io, room);
        }
      }
    } else {
      // Wrong step
      room.gameState.eliminatedIds.push(socket.id);
